...
class Solution {
    ...
    class Automaton {
        ...
        public void get(char c) {
            this.state = this.table.get(this.state)[get_col(c)];
            if ("in_number".equals(this.state)) {
                number = number * 10 + (c - '0');
                number = sign == 1 ? Math.min(number, (long) Integer.MAX_VALUE) : Math.min(number, -(long) Integer.MIN_VALUE);
